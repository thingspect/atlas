...
func gatewayStats(body []byte) ([]*decode.Point, error) {
	...
	if statsMsg.Time != nil {
		msgs = append(msgs, &decode.Point{Attr: "time",
			Value: strconv.FormatInt(statsMsg.Time.Seconds, 10)})
	}
	if statsMsg.RxPacketsReceived != 0 {
		msgs = append(msgs, &decode.Point{Attr: "rx_received",
			Value: int32(statsMsg.RxPacketsReceived)})
	}
	if statsMsg.RxPacketsReceivedOk != 0 {
		msgs = append(msgs, &decode.Point{Attr: "rx_received_valid",
			Value: int32(statsMsg.RxPacketsReceivedOk)})
	}
	if statsMsg.TxPacketsReceived != 0 {
		msgs = append(msgs, &decode.Point{Attr: "tx_received",
			Value: int32(statsMsg.TxPacketsReceived)})
	}
	if statsMsg.TxPacketsEmitted != 0 {
		msgs = append(msgs, &decode.Point{Attr: "tx_transmitted",
			Value: int32(statsMsg.TxPacketsEmitted)})
	}
	...
	for k, v := range statsMsg.MetaData {
		...
		case len(keys) == 2 && keys[0] == "int":
			...
			msgs = append(msgs, &decode.Point{Attr: keys[1],
				Value: int32(intVal)})
