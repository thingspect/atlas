...
// mutexCache contains methods to create and query data in memory and
// implements the Cacher interface.
type mutexCache[V Cacheable] struct {
	cache *ttlcache.Cache[string, V]
}

// Verify mutexCache implements Cacher.
var _ Cacher[string] = &mutexCache[string]{}

// NewMutex builds a new Cacher and returns it.
func NewMutex[V Cacheable]() Cacher[V] {
	cache := ttlcache.New(
	...
	return &mutexCache[V]{cache: cache}
}

// Set sets key to value.
func (m *mutexCache[V]) Set(ctx context.Context, key string, value V) error {
	return m.SetTTL(ctx, key, value, ttlcache.NoTTL)
}

// SetTTL sets key to value with expiration.
func (m *mutexCache[V]) SetTTL(_ context.Context, key string, value V,
	exp time.Duration,
) error {
...
// returned.
func (m *mutexCache[V]) Get(_ context.Context, key string) (V, error) {
...
func (m *mutexCache[V]) SetIfNotExist(ctx context.Context, key string,
	value V,
) error {
...
func (m *mutexCache[V]) SetIfNotExistTTL(_ context.Context, key string,
...
func (m *mutexCache[V]) Incr(_ context.Context, _ string) (int64, error) {
	panic("unimplemented")
}

// Del removes the specified key. A key is ignored if it does not exist.
func (m *mutexCache[V]) Del(_ context.Context, key string) error {
	m.cache.Delete(key)
...
func (m *mutexCache[V]) Close() error {
	m.cache.Stop()
