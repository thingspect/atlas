...
func TestGatewayStats(t *testing.T) {
	...
	tests := []struct {
		...
		// Gateway Stats.
		{&gw.GatewayStats{}, []*parse.Point{
			{Attr: "raw_gateway", Value: `{}`},
		}, ""},
		{&gw.GatewayStats{GatewayId: bUniqID, Ip: "127.0.0.1", Time: pNow,
			RxPacketsReceived: 1, RxPacketsReceivedOk: 2, TxPacketsReceived: 3,
			TxPacketsEmitted: 4, MetaData: map[string]string{"aaa": "bbb"}},
			[]*parse.Point{
