...
func TestParseGateways(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(fmt.Sprintf("Can parse %+v", lTest), func(t *testing.T) {
			...
			require.NoError(t, err)
			parserPubGWTopic := "topic-" + random.String(10)

			ing := Ingestor{
				mqttGWSub: mqttGWSub,

				parserQueue:      parserQueue,
				parserPubGWTopic: parserPubGWTopic,
			}
			go func() {
				ing.parseGateways()
			}()
			...
			for _, res := range lTest.res {
				...
				case msg := <-parserSub.C():
					...
					t.Logf("msg.Topic, msg.Payload: %v, %s", msg.Topic(),
						msg.Payload())
					require.Equal(t, parserPubGWTopic, msg.Topic())
...
func TestParseGatewaysError(t *testing.T) {
	t.Parallel()
...
func TestParseDevices(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(fmt.Sprintf("Can parse %+v", lTest), func(t *testing.T) {
			...
			parserSub, err := parserQueue.Subscribe("")
			require.NoError(t, err)
			parserPubDevTopic := "topic-" + random.String(10)
			parserPubDataTopic := "topic-" + random.String(10)

			ing := Ingestor{
				mqttDevSub: mqttDevSub,

				parserQueue:        parserQueue,
				parserPubDevTopic:  parserPubDevTopic,
				parserPubDataTopic: parserPubDataTopic,
			}
			go func() {
				ing.parseDevices()
			}()
			...
			for _, res := range lTest.res {
				...
				case msg := <-parserSub.C():
					...
					t.Logf("msg.Topic, msg.Payload: %v, %s", msg.Topic(),
						msg.Payload())
					require.Equal(t, parserPubDevTopic, msg.Topic())
...
func TestParseDevicesError(t *testing.T) {
	t.Parallel()
...
func TestPointToVIn(t *testing.T) {
	...
	tests := []struct {
		...
		{&parse.Point{Attr: "data_rate", Value: int32(3)}, &message.ValidatorIn{
			Point: &common.DataPoint{UniqId: uniqID, Attr: "data_rate",
				ValOneof: &common.DataPoint_IntVal{IntVal: 3}, Ts: now,
				TraceId: traceID}, SkipToken: true}},
		{&parse.Point{Attr: "data_rate", Value: 3}, &message.ValidatorIn{
			Point: &common.DataPoint{UniqId: uniqID, Attr: "data_rate",
				ValOneof: &common.DataPoint_IntVal{IntVal: 3}, Ts: now,
				TraceId: traceID}, SkipToken: true}},
		{&parse.Point{Attr: "data_rate", Value: int64(3)}, &message.ValidatorIn{
			Point: &common.DataPoint{UniqId: uniqID, Attr: "data_rate",
				ValOneof: &common.DataPoint_IntVal{IntVal: 3}, Ts: now,
				TraceId: traceID}, SkipToken: true}},
		{&parse.Point{Attr: "snr", Value: 7.8}, &message.ValidatorIn{
			Point: &common.DataPoint{UniqId: uniqID, Attr: "snr",
				ValOneof: &common.DataPoint_Fl64Val{Fl64Val: 7.8}, Ts: now,
				TraceId: traceID}, SkipToken: true}},
		{&parse.Point{Attr: "snr", Value: float32(7.0)}, &message.ValidatorIn{
			Point: &common.DataPoint{UniqId: uniqID, Attr: "snr",
				ValOneof: &common.DataPoint_Fl64Val{Fl64Val: 7.0}, Ts: now,
				TraceId: traceID}, SkipToken: true}},
		{&parse.Point{Attr: "ack", Value: "OK"}, &message.ValidatorIn{
			Point: &common.DataPoint{UniqId: uniqID, Attr: "ack",
				ValOneof: &common.DataPoint_StrVal{StrVal: "OK"}, Ts: now,
				TraceId: traceID}, SkipToken: true}},
		{&parse.Point{Attr: "adr", Value: false}, &message.ValidatorIn{
			Point: &common.DataPoint{UniqId: uniqID, Attr: "adr",
				ValOneof: &common.DataPoint_BoolVal{BoolVal: false}, Ts: now,
				TraceId: traceID}, SkipToken: true}},
		{&parse.Point{Attr: "ack", Value: []byte{0x00}}, &message.ValidatorIn{
			Point: &common.DataPoint{UniqId: uniqID, Attr: "ack",
				ValOneof: &common.DataPoint_BytesVal{BytesVal: []byte{0x00}},
				Ts:       now, TraceId: traceID}, SkipToken: true}},
		{&parse.Point{Attr: "error", Value: io.EOF}, &message.ValidatorIn{
